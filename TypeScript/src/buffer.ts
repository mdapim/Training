export default class CircularBuffer<T> {
  ...
  private bufferArr: string[] | undefined[];
  ...
  write(value: string): void {
    if (
      this.bufferArr[this.currentIndex] === undefined &&
      this.currentIndex <= this.initialValue - 1
    ) {
      this.bufferArr[this.currentIndex] = value;
      this.currentIndex += 1;
      if (this.currentIndex > this.initialValue - 1) this.currentIndex = 0;
    } else throw new BufferFullError("error adding new value");
  }
  ...
  read(): string | undefined {
    ...
    if (this.bufferArr[this.lastIndex] === undefined) {
      ...
      const value: string | undefined = this.bufferArr[this.lastIndex];
  ...
  checkForLoop(): void {
    if (this.lastIndex > this.initialValue - 1) this.lastIndex = 0;
  }
}
